...
def createUser(import_user):
    ...
    trace("\n* Cardholders - Create")
    new_cardholder = { "FirstName": import_user["FirstName"], "IdNumber" : import_user["IdNumber"] , "CHType": import_user["CHType"], \
                        "PartitionID": import_user["PartitionID"], "CHState" : import_user["CHState"], "AuxText05" : import_user["AuxText05"], \
                        "AuxText08" : import_user["AuxText08"], "CHEndValidityDateTime" : end_validity_str, \
                        "AuxText15" : import_user["AuxText15"] } 
...
def assign_access_level(wxs_user, all_access_levels):
    ...
    for access_level in all_access_levels:
        if wxs_user["AuxText15"] == access_level["AccessLevelName"]:
            ...
            assign = requests.post(WxsConn.waccessapi_endpoint + f'cardholders/{wxs_user["CHID"]}/accessLevels/{access_level["AccessLevelID"]}', headers=WxsConn.waccessapi_header, json={})
            return
    # GroupID 1 = Usuários sem nivel de acesso
    set_group = requests.post(WxsConn.waccessapi_endpoint + f'cardholders/{wxs_user["CHID"]}/groups/1', headers=WxsConn.waccessapi_header)
...
def set_chtype(tipo):
    if tipo.upper() == 'MEDICO(A)':
        trace(f"Set CHType = {6} [MEDICO(A)]")
        return(6)
    else:
        trace(f"Set CHType = {5} [{tipo}]")
        return(5)