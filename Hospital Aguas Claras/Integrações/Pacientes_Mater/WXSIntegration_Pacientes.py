...
def check_user(import_user):
    ...
    try:
        ...
        if menor_idade and import_user_lst["cd_multi_empresa"] == 8:
            partition_id = 4
            partition_desc = '[HAC - Paciente] Pediatrico'
        elif menor_idade == False and import_user_lst["cd_multi_empresa"] == 8:
            partition_id = 2
            partition_desc = '[HAC - Paciente] Adulto'
        elif menor_idade and import_user_lst["cd_multi_empresa"] == 6:
            partition_id = 10
            partition_desc = '[MTN - Paciente] Pediatrico'
        elif menor_idade == False and import_user_lst["cd_multi_empresa"] == 6:
            partition_id = 9
            partition_desc = '[MTN - Paciente] Adulto'

        #partition_desc = '[Paciente] Adulto' if partition_id == 2 else '[Paciente] Pediatrico' if partition_id == 2 else None
        
        # if partition_id not in (2, 4, 9, 10):
        #     partition_desc = None

        trace(f'Set PartitionID to user: [{partition_desc}] | user is menor de idade: {menor_idade} | cd_multi_empresa: {import_user_lst["cd_multi_empresa"]}')
        ...
        if wxs_user_list:
            ...
            for wxs_user in wxs_user_list:
                ...
                if fields_with_difference:
                    ...
                    if wxs_user["AuxLst04"]:
                        assign_access_level(wxs_user)
                        assign_access_level_contact(wxs_user)
                    else:
                        trace('Nenhum Nivel de acesso Atribuido, usuário fora da Regra de Internação')
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            reply = requests.get(url + f'atendimento', headers = headers_2, params=(("tipo_atendimento", "I"),("dt_alta_hospitalar", "null"), ("atendimento_pai", "null")))
        ...
        if firts_iteration and WxsConn.dump_firts_iteration:
            ...
            for api_user in import_list:
                if api_user["cd_multi_empresa"] == 5:
                    continue
                # if api_user["atendimento_pai"]:
                #     continue
