...
def new_attendence(wxs_user, import_user):
    if wxs_user["ActiveVisit"]:
    ...
    get_card = requests.get(WxsConn.waccessapi_endpoint + 'cards', headers=WxsConn.waccessapi_header, params=(("limit", 1),("cardType", 1),("situation", 2), ("ClearCode", f'BIO_{wxs_user["CHID"].zfill(9)}' )))
...
def set_status(import_user_lst):
    ...
    elif import_user_lst["tipo_saida"].upper() == 'INTERNACAO':
