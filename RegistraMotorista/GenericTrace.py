...
def trace(Message, userID='', color='white', print_to_oper=False):
...
def report_exception(e):
    ...
    with open('ErrorLog.txt', 'a') as file:  # Use file to refer to the file object
        file.write("Bypassing exception at %s (%s)" % (t, e))
        file.write("**** Exception: <code>%s</code>" % (traceback.format_exc(),))
...
def write_txt(message):
    ...
    folderName = 'Trace ' + executableName 
    os.makedirs(folderName, exist_ok=True)
    f = os.path.isfile(folderName + '/log_integra.txt')
    if f:
        list_of_files = os.listdir(folderName + '/')
        ...
        fileMaxSize = 1000000
        size = os.path.getsize(folderName + '/log_integra.txt')
        if size > fileMaxSize:
            Current_Date = datetime.now().strftime ('%Y-%m-%d_%H_%M_%S')
            os.rename(folderName + '/log_integra.txt', folderName + '/'+ str(Current_Date) + ' - log_integra.txt')
            with open(folderName + "/log_integra.txt", "a", encoding="utf-8") as newfile:
                newfile.write('')

        with open(folderName + "/log_integra.txt", "a", encoding="utf-8") as myfile:
            myfile.write(f"\n{datetime.now().strftime ('%Y-%m-%d %H:%M:%S.%f')[:-3]} - {message}")
